...
class PMSTestSuite(unittest.TestCase):
    ...
    def testCRUDFase(self):
        ...
        self.testRoute(self.eliminarProyectoURL + getProyecto(nombreProyecto).id.__str__(), self.helperEliminar('Proyecto', nombreProyecto))
        rv = self.app.post(self.eliminarProyectoURL,follow_redirects=True)
        assert self.listProject in rv.data
        assert nombreProyecto not in rv.data
